...
namespace GotaSequenceLib.Playback {
    public class Mixer : IDisposable {
        ...
        public Channel AllocateChannel(InstrumentType type, Track track) {
            ...
            switch (type) {
                case InstrumentType.PCM: allowedChannels = 0b1111111111111111; break; 
                case InstrumentType.PSG: allowedChannels = 0b0011111100000000; break; 
                case InstrumentType.Noise: allowedChannels = 0b1100000000000000; break; 
                default: return null;
            }
        ...
        public void ChannelTick() {
            for (int i = 0; i < 0x10; i++) {
                ...
                if (chan.Owner != null) {
                    ...
                    int pitch = ((chan.Key - chan.BaseKey) << 6) + chan.SweepMain() + chan.Owner.GetPitch(); 
