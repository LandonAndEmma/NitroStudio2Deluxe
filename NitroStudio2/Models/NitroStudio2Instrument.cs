...
{
    /// <summary>
    /// The .ns2i instrument file, which carries its samples alongside the instrument so it can
    /// be moved between banks. Ported unchanged from the WinForms project.
    /// </summary>
    public class NitroStudio2Instrument : IOFile
    {
        ...
        public NitroStudio2Instrument(
            ...
            ushort war2,
            ushort war3,
            Func<RiffWave, List<WaveArchiveInfo>, Task<ushort?>> mapWave = null
        )
        {
            Inst = inst;
            if (s == null)
        ...
        public async Task<bool> WriteInstrument(
            Bank bnk,
            int instrumentId,
            SoundArchive a,
            ushort war0,
        ...
        )
        {
            ...
            foreach (NoteInfo r in Inst.NoteInfo)
            {
                ...
                if (!found)
                {
                    ...
                    // The caller shows the wave mapper and tells us which archive to use.
                    ushort? chosen = mapWave is null
                        ? null
                        : await mapWave(riff, wars.Where(x => x != null).ToList());
                    if (chosen is null)
                    {
                        return false;
                    }
                    ushort target = chosen.Value;
                    a.WaveArchives.Where(x => x.Index == target)
                        .FirstOrDefault()
                        .File.Waves.Add(e.Wave);
                    r.WaveId = (ushort)(
                        a.WaveArchives.Where(x => x.Index == target).FirstOrDefault()
                            .File.Waves.Count() - 1
                    );
                    r.WarId = (ushort)
                        wars.ToList()
