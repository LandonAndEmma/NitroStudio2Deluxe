...
namespace Kermalis.SoundFont2
{
    public sealed class InfoListChunk : SF2ListChunk
    {
        ...
        public string? Comment
        {
            ...
            set
            {
                ...
                if (value is not null)
                {
                    ...
                    {
                        _subChunks.Add(
                            new HeaderSubChunk(_sf2, "ICMT", value, maxSize: COMMENT_MAX_SIZE)
                        );
                    }
                }
                else
        ...
        internal InfoListChunk(SF2 inSf2)
            : base(inSf2, "INFO")
        {
        ...
        internal InfoListChunk(SF2 inSf2, EndianBinaryReader reader)
            : base(inSf2, reader)
        {
            _subChunks = new List<SF2Chunk>();
            long startOffset = reader.Stream.Position;
            while (reader.Stream.Position < startOffset + Size - 4)
            {
                ...
                switch (name)
                {
                    case "ICMT":
                        _subChunks.Add(
                            new HeaderSubChunk(inSf2, reader, maxSize: COMMENT_MAX_SIZE)
                        );
                        break;
                    case "ifil":
                    case "iver":
                        _subChunks.Add(new VersionSubChunk(inSf2, reader));
                        break;
                    case "isng":
                    case "INAM":
                    case "ICRD":
                    ...
                    case "ISFT":
                    case "irom":
                        _subChunks.Add(new HeaderSubChunk(inSf2, reader));
                        break;
                    default:
                        throw new NotSupportedException(
                            $"Unsupported chunk name at 0x{reader.Stream.Position:X}: \"{name}\""
                        );
                }
            }
        }
