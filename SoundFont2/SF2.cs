...
namespace Kermalis.SoundFont2
{
    public sealed class SF2
    {
            ...
            using (FileStream stream = File.Open(path, FileMode.Open))
            {
                ...
                if (str != "RIFF")
                {
                    throw new InvalidDataException(
                        "RIFF header was not found at the start of the file."
                    );
                }
                ...
                {
                    throw new InvalidDataException(
                        "sfbk header was not found at the expected offset."
                    );
                }
        ...
        {
            return HydraChunk.INSTSubChunk.AddInstrument(
                new SF2Instrument(name, (ushort)HydraChunk.IBAGSubChunk.Count)
            );
        }
        ...
        {
            HydraChunk.PHDRSubChunk.AddPreset(
                new SF2PresetHeader(name, preset, bank, (ushort)HydraChunk.PBAGSubChunk.Count)
            );
        }
        ...
        {
            return HydraChunk.SHDRSubChunk.AddSample(
                new SF2SampleHeader(
                    name,
                    start,
                    end,
        ...
        internal void UpdateSize()
        {
            ...
            _size =
                4
                + InfoChunk.UpdateSize()
                + 8
                + SoundChunk.UpdateSize()
                + 8
                + HydraChunk.UpdateSize()
                + 8;
        }
    }
}
